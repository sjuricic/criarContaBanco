<script type='text/javascript'>
	...
	function taskFormValidator() {
		...
		var myInputs = new Array();
					myInputs[i] = document.getElementById("gerente_t");
					i++;


		var j=0;
						if(notEmpty(myInputs[j]) && !isAlphanumeric(myInputs[j])) {
							alert("Please enter valid gerente_t");
							myInputs[j].focus();
							return false;
						}
...
<div id="container">
	<div id="header">
		User Task Form: createBankAccount.AprovaGerente
	</div>
	<div id="content">
		...
		<fieldset>
            <legend>Task Inputs</legend>
                            		...
                            		<div class="div_checkbox">
                              		<![CDATA[[from:|tousers:brmsuser|togroups:|replyTo:|subject:Finalizar a Task|body:Finalizar a Task]@[20s]]]>
                            		</div>
                            		<label for="name">NotStartedNotify</label>
                            		<div class="div_checkbox">
                              		<![CDATA[[from:|tousers:brmsuser|togroups:|replyTo:|subject:Iniciar a Task|body:Iniciar a Task]@[15s]]]>
                            		</div>
          ...
          <fieldset>
            <legend>Task Outputs</legend>
                            		<label for="name">gerente_t</label>
                            		<div class="div_checkbox">
                              		<input name="gerente_t" type="checkbox" class="checkbox" id="gerente_t" value="${gerente_t}" />
                            		</div>
